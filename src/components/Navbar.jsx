...
import { FaHeart, FaSearch, FaShoppingBag, FaTimes } from "react-icons/fa"

const Navbar = ({ searchQuery, onSearch, cartItems, onRemoveFromCart, onChangeQuantity, favoriteItems, onRemoveFavorite, onAddToCart }) => {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[#deded8] bg-[#f4f3ef]/95 shadow-sm">
        <Container>
          <div className="flex h-[72px] items-center justify-between sm:h-[88px]">
            ...
            <div className="flex items-center gap-[21px]">
              ...
              <button className="relative border-0 bg-transparent p-[5px] text-[#1a1b19] transition-colors hover:text-[#6d9f68]" type="button" aria-label="Seçilmiş məhsullara bax" onClick={() => setIsFavoritesOpen(true)}><FaHeart className="text-base" />{favoriteItems.length > 0 && <span className="absolute -right-2 -top-1 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-[#d8e24a] text-[10px]">{favoriteItems.length}</span>}</button>
        ...
        {isFavoritesOpen && <FavoritesDrawer items={favoriteItems} onClose={() => setIsFavoritesOpen(false)} onRemove={onRemoveFavorite} onAddToCart={onAddToCart} />}
    </header>
  )
}
